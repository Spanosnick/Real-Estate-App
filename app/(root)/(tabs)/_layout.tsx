...
const TabIcon = ({ focused ,icon , title}: {focused :boolean,icon: any,title:string})  => {
    const standardClasses = 'text-xs w-full text-center mt-1';
    return (
        <View className='flex-1 mt-3 flex flex-col items-center'>
            <Image source={icon} style={{width: 20, height: 20, tintColor: focused ? '#0061ff' : '#666876'}}
                   resizeMode='contain' className='size-3'/>
            <Text className={`${focused ? 'text-primary-300 font-rubik-medium'  :
                'text-black-200 font-rubik'} ${standardClasses}` }>
                {title}
            </Text>
        </View>
    )
