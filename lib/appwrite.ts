...
export  async function login(){
    ...
    try {
        ...
        if (!secret || !userId) {
            throw new Error('Failed to login 3');
        }

        const session = await account.createSession(userId,secret);

        if (!session) {
            throw new Error('Failed to login 4');
        }
        return true;
...
export async function getLatestProperties(){
    try {
        const result = await databases.listDocuments(config.databaseId!,
